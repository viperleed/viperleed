...
def R_zj(
    ...
    exp_mask = dnl.xp.isnan(exp_intensity)
    exp_intensity = dnl.xp.where(exp_mask, 0.0, exp_intensity)
    exp_derivative_1 = dnl.xp.where(exp_mask, 0.0, exp_derivative_1)
    exp_derivative_2 = dnl.xp.where(exp_mask, 0.0, exp_derivative_2)
    ...
    theo_derivative_1 = dnl.xp.where(mask, 0.0, theo_derivative_1)
    theo_derivative_2 = dnl.xp.where(mask, 0.0, theo_derivative_2)
    ...
    denominators = abs(exp_derivative_1) + dnl.xp.nanmax(
        exp_derivative_1, axis=0
    )
