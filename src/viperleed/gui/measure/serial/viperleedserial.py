"""Module viperleedserial of viperleed.gui.measure.serial.

This module contains the definitions of the ViPErLEEDSerial class and
its associated ViPErLEEDErrorEnum class ViPErLEEDHardwareError that are
used for serial communication with the Arduino Micro controller used by
ViPErLEED. The serial communication happens in a separate thread to
prevent stalling the Graphical User Interface.
"""
...
class ViPErLEEDSerial(SerialABC):
    ...
    def __firmware_and_hardware(self, message):
        ...
        if firmware_version > local_version:
            # If the firmware version of the controller is greater than
        ...
        # case the firmware version in the hardware is older than the
        # one of the settings.
