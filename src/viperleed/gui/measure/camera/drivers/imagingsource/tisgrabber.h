...
//////////////////////////////////////////////////////////////////////////
/*! @mainpage Please see "tisgrabber.h" for the function documentation. */
...
/*! This error indicates, that an HGRABBER handle has not been created yet.
    Please see IC_CreateGrabber() for creating an HGRABBER handle. */
#define IC_NO_HANDLE -1        ///< No device handle. HGRABBER is NULL.


/*! This return values indicates that no device has been opened. Please refer
    to IC_OpenVideoCaptureDevice(). */
...
#define IC_NOT_AVAILABLE -3     ///< Property not available, but HGRABBER is valid.


/*! This return value indicates, that the property set was not queried for
    the current grabber handle. Please check, whether IC_QueryPropertySet()
    was called once before using the function. */
#define IC_NO_PROPERTYSET -3     ///< The Property Set was not queried.


/*! This return value indicates, that setting of a custom live display window
    size failed, because IC_SetDefaultWindowPosition() was not called with
    parameter false somewhere before.
...
/*! This return value indicates, that the video capture device is not in live
    mode, but live mode is for the current function call required. Please refer
    to IC_StartLive(). */
#define IC_NOT_IN_LIVEMODE -3 ///< A device has been opened, but is is not in live mode.


/*! This return value indicates, that a device does not support the requested
    property, or the name of a property was written in wrong way.
...
/*! This return value indicates, that a device does not support the requested
    element property, or the name of an element was written in wrong way.
...
/*! This return value indicates, that a property element does not support
    ...
    IC_GetPropertyValueRange(hGrabber, "Exposure","Auto", &min, &max )
...
/*! This return value indicates, that that the passed XML file contains no
    valid XML data.
...
/*! Initialize the ICImagingControl class library. This function must be called
    ...
    @param szLicenseKey IC Imaging Control license key or NULL if only a
...
/*! Open a video capture device. The hGrabber handle must have been created
    previously by a call to IC_CreateGrabber(). Once a hGrabber handle has
    been created it can be recycled to open different video capture devices
    in sequence.
    @param hGrabber The handle to grabber object, that has been created by a
                    call to IC_CreateGrabber
    @param szDeviceName Friendly name of the video capture device
                        e.g. "DFK 21F04".
    @retval IC_SUCCESS on success.
...
/*! Close the current video capture device. The HGRABBER object will not be
...
/*! Return the serial number of the current device. Memory for the serial
    number must have been allocated by the application:
    ...
    @retval IC_IC_NOT_AVAILABLE The video capture device does not provide
                                a serial number.
...
/*! Count all connected video capture devices. If the Parameter szDeviceList
    ...
    @param szDeviceList
        A two dimensional char array that receives the list. Or NULL if only
        the count of devices is to be returned.
    @param iSize
        Not used.
...
/*! Simpler approach of enumerating devices. No 2D char array needed
    ...
    @endcode
    @param szDeviceName
        Char memory, that receives the device name
    @param iSize
        Size of the char memory. If names are longer, they will be truncated.
    @param DeviceIndex
...
/*! Get the number of the currently available devices. This function creates
    an internal array of all connected video capture devices. With each call
    to this function, this array is rebuilt. The name and the unique name can
    be retrieved from the internal array using the functions IC_GetDevice()
    and IC_GetUniqueNamefromList. They are useful for retrieving device names
    for opening devices.
...
/*! Get a string representation of a device specified by iIndex. iIndex
    ...
    @param iIndex The number of the device whose name is to be returned.
                  It must be in the range from 0 to IC_GetDeviceCount(),
    @return Returns the string representation of the device on success,
            NULL otherwise.
...
/*! Get unique device name of a device specified by iIndex. The unique device
    name consist of the device name and its serial number. It allows to differ
    between more then one device of the same type connected to the computer.
    The unique device name is passed to the function IC_OpenDevByUniqueName

    @param iIndex
        The number of the device whose name is to be returned. It must be
        in the range from 0 to IC_GetDeviceCount(),
    @return
        Returns the string representation of the device on success, NULL
        otherwise.
...
/*! Open a video capture by using its DisplayName.
    @param hGrabber
        The handle to the grabber object.
    @param szDisplayName
        DisplayName of the device. Can be retrieved by a call
...
/*! Get a DisplayName from a currently open device. The display name of a
    ...
    @param hGrabber
        Handle to a grabber object
    @param szDisplayName
        Memory that will take the display name. If it is NULL, the
        length of the display name will be returned.
    @param iLen
        Size in Bytes of the memory allocated by szDisplayName.


    @retval IC_SUCCESS
        On success. szDisplayName contains the display name of the device.
    @retval IC_ERROR
        iLen is less than the length of the retrieved display name.
    @retval IC_NO_HANDLE
        hGrabber is not a valid handle. GetGrabber was not called.
    @retval IC_NO_DEVICE
        No device opened. Open a device, before this function can be used.
    @retval > 1
        Length of the display name, if szDisplayName is NULL.
...
/*! Get a UniqueName from a currently open device.
    ...
    @param hGrabber
        Handle to a grabber object
    @param szUniqueName
        Memory that will take the Unique name. If it is NULL, the
        length of the Unique name will be returned.
    @param iLen
        Size in Bytes of the memory allocated by szUniqueName.

    @retval IC_SUCCESS
        On success. szUniqueName contains the Unique name of the device.
    @retval IC_ERROR
        iLen is less than the length of the retrieved Unique name.
    @retval IC_NO_HANDLE
        hGrabber is not a valid handle. GetGrabber was not called.
    @retval IC_NO_DEVICE
        No device opened. Open a device, before this function can be used.
    @retval > 1
        Length of the Unique name, if szUniqueName is NULL. */
...
/*! This returns 1, if a valid device has been opened, otherwise it is 0.
    ...
    @retval IC_ERROR
        There is no valid video capture device opened
    @retval IC_SUCCESS
        There is a valid video capture device opened. */
...
/*! Show the device selection dialog. This dialogs enables to select the
    ...
    @return
        The passed hGrabber object or a new created if hGrabber was NULL.
    ...
    if( hTheGrabber != NULL )
    {
        // Show the live video of this grabber
        IC_StartLive( hTheGrabber, 1 );
        // Show the property page of this grabber
        IC_ShowPropertyDialog( hTheGrabber );
    }
    @endcode */
...
/*! Set a video format for the current video capture device. The video format
    ...
    @param hGrabber
        The handle to the grabber object.
    @param szFormat
        A string that contains the desired video format.
    @retval IC_SUCCESS
        on success
    @retval IC_ERROR
...
/*! Count all available video formats.
    If the Parameter szFormatList is NULL, only the number of formats is
    queried. The Parameter szFormatList must be a two dimensional array of
    char. The iSize parameter specifies the length of the strings, that are
    used in the array to store the format names.

    @param hGrabber
        The handle to the grabber object.
    @param szFormatList
        A two dimensional char array that receives the list,
        or NULL if only the count of formats is to be returned.

    @retval >= 0
        Success, count of found video formats
    @retval < 0
        An error occurred.
...
/*! Simpler approach of enumerating video formats. No 2D char array needed.
    ...
    @param hGrabber
        The handle to the grabber object.
    @param szFormatName
        char memory, that will receive the name of the video format.
        Should be big enough.
    @param iSize
        Size in byte of szFormatName
    @param iIndex
        Index of the video format to query. Must be between 0
        and IC_GetDeviceCount.
    ...
    @retval IC_SUCCESS
        Success,
    @retval IC_NO_DEVICE
        No video capture device selected.
    @retval IC_NO_HANDLE
        No handle to the grabber object. */
...
/*! Get a string representation of the video format specified by iIndex.
    ...
    @param hGrabber
        The handle to the grabber object.
    @param iIndex
        Number of the video format to be used.

    @retval Non-NULL
        The name of the specified video format.
    @retval NULL
        An error occurred.
...
// ###########################    SINK FORMAT    ##############################
//
// Sink format refers to the way frames are internally stored in the camera
// hardware. According to the documentation, digitized frames are duplicated
// over two streams: one stream goes to the PC and has VIDEO FORMAT, the
// other goes to the camera internal memory and has SINK FORMAT.
//
// I (Michele Riva) believe it is likely best to always keep both formats
// consistent (at least concerning data types), although the re is no strict
// requirement for them to be the same.

/*! Set the sink type. A sink type must be set before images can be snapped.
    The sink type basically describes the format of the buffer where the
    snapped images are stored.
    ...
    @param hGrabber
        The handle to the grabber object.
    @param format
        The desired color format. Possible values for format are:
        @li Y800
        @li RGB24
        @li RGB32
        @li UYVY
    ...
    @note Please note that UYVY can only be used in conjunction with
          a UYVY video format. */
int AC IC_SetFormat( HGRABBER hGrabber, COLORFORMAT format ); ///< Sets the color format of the sink.


/*! Retrieve the format of the sink type currently set
    (See IC_SetFormat() for possible formats).
    If no sink type is set or an error occurred, NONE is returned.
    The function returns a valid value only after IC_PreprareLive()
    or IC_StartLive() was called. Before these calls, NONE is returned.

    @param hGrabber
        The handle to the grabber object.
    @return
        The current sink color format. */
...
// ############################   VIDEO NORM   ################################
//
// As far as I could understand, this refers to the PAL/NTSC/... norm.
// It is unclear whether we really need to do anything with this, but it
// may be necessary in case it has any impact on the 'base frame rate'
// (e.g., 50Hz vs 60Hz), as this may have an effect on the line-frequency
// rejection ability of the camera.

// Some notes from the documentation:
// -- The video norm depends on the current device. So changing the
//    ICImagingControl.Device property, will invalidate the video norm.
// -- Changing the video norm invalidates the ICImagingControl.VideoFormat
//    property.
// -- Note that video norms may not be supported on all video capture
//    devices. A video camera that is connected to a frame grabber usually
//    supports either NTSC or PAL as a video norm, and the correct one must
//    be selected in order to get an image from the grabber. Cameras that are
//    connected directly via USB or FireWire ports usually do not use a video
//    norm. The ICImagingControl.VideoNormAvailable property can be used to
//    determine whether video norms are supported.
...
/*! Get a string representation of the video norm specified by iIndex.
    ...
    @retval Non-NULL The name of the specified video norm.
...
/*! Start the live video.
    ...
    @param iShow The parameter indicates:
        @li 1 : Show the video
        @li 0 : Do not show the video, but deliver frames. (For callbacks etc.)
    @retval IC_SUCCESS on success
    @retval IC_ERROR if something went wrong.
    @sa IC_StopLive */
...
/*! Check, whether the passed grabber already provides are live video
    @param hGrabber The handle to the grabber object.
    @retval 1 : Live video is running, 0 : Live video is not running.
...
/*! Sets a new frame rate.
    ...
    @retval IC_NOT_AVAILABLE The property is not supported by the
                             current device
    @retval IC_NO_HANDLE    Invalid grabber handle
    @retval IC_NO_DEVICE    No video capture device opened
    @retval IC_NOT_IN_LIVEMODE Frame rate can not set, while live video is
...
/*! Retrieves the current frame rate
    ...
    @param hGrabber    Handle to a grabber object.
    @retval The current frame rate. If it is 0.0, then frame rates are
            not supported. */
...
/*! SignalDetected
    ...
    @retval IC_NO_HANDLE  Invalid grabber handle
    @retval IC_NO_DEVICE    No video capture device opened
    @retval IC_NOT_IN_LIVEMODE  No live mode, IC_StartLive() was not called */
...
/*! Save the contents of the last snapped image by IC_SnapImage into a file.
    ...
    is set to Y800, the saved image will be an 8 Bit gray-scale image. In any
...
/*! Retrieve a byte pointer to the image data (pixel data) of the last snapped
    ...
    @param hGrabber The handle to the grabber object.
    @retval Non-NULL Pointer to the image data
    @retval NULL Indicates that an error occurred.
    @sa IC_SnapImage
    @sa IC_SetFormat */
unsigned char* AC IC_GetImagePtr( HGRABBER hGrabber ); ///< Returns a pointer to the image data
...
/*! Enables or disables the default window size lock of the video window.
    ...
    @param hGrabber    Handle to a grabber object.
    @param Default    0 = disable, custom size can be set, 1 = enable, the standard size, which is video format, is used.
    ...
    @retval IC_NO_HANDLE        Invalid grabber handle
    @retval IC_NO_DEVICE        No video capture device open */
...
/*! Check, whether a property is supported by the current video capture device.
    @param hGrabber The handle to the grabber object.
    @sa eProperty The camera property to be checked
...
//
//  Out of these, perhaps the only one that may make sense to use
//  is GAIN, although it is unclear whether these properties merely
//  affect post-processing of the acquired frames done internally in
//  the firmware, and not ACQUISITION itself.
...
/*! Enable or disable automatic for a video property.
    @param hGrabber The handle to the grabber object.
    @param iProperty The property to be set. It can have following values:
    @li PROP_VID_BRIGHTNESS,
    @li PROP_VID_CONTRAST,
    ...
    automation of the property is not available with the current video capture
    device, the function returns 0. On success, the function returns 1. */
int AC IC_EnableAutoVideoProperty( HGRABBER hGrabber, int iProperty, int iOnOff ); ///< Switch automation for a video property,
...
// ##########################   INPUT CHANNELS   ##############################
// Seems like some cameras have analog and/or digital input channels
// E.g., 33U has a DI that can be used for triggering, and a DO that
// can be configured as STROBE (active == exposure starts)
...
/*! Get a string representation of the input channel specified by iIndex.
    ...
    @retval Non-NULL The name of the specified input channel
    @retval NULL An error occurred.
...
/*! Load a device settings file.
    ...
    @param hGrabber The handle to the grabber object. If it is NULL then a new HGRABBER handle is
                    ...
                    open in the grabber handle. The properties in the passed XML file will be applied to
                    the opened device.
...
/*! Get trigger modes.
    ...
    @param hGrabber      Handle to a grabber object.
    @param szModeList    Two-dimensional array of char that will receive the mode list.
...
/*! Set the trigger polarity
    ...
    @param hGrabber    Handle to a grabber object.
    @param iPolarity
        @li 0 : Polarity on direction (== rising edge)
        @li 1 : Polarity the other direction (== falling edge)
...
//  THERE ALSO SEEMS TO BE A 'TRIGGER BURST COUNT' for taking multiple
//  frames with a single trigger pulse.
//  VCDID_Trigger\VCDElement_TriggerBurstCount

//  Also, a VCDID_Trigger\VCDElement_TriggerDelay, but it is only for
//  hardware trigger it seems.
...
/*!    Enable frame ready callback.
    ...
    @param x1_argument_in_void_user data Pointer to some user data.
...
int AC IC_SetFrameReadyCallback(
    ...
    void*                    x1_argument_in_void_user data);
...
int AC IC_SetCallbacks(
    ...
    void*                    x2_argument_in_void_user data);
...
/*! Resets the driver. Do not use, for internal purposes only.
...
/*!  BeginPaint returns an HDC for GDI painting purposes (like TextOut() etc.)
    ...
    @return HDC The function returns not NULL, if the HDC could be retrieved. If the HDC
            could not be retrieved or an error has occurred, the function returns 0.
...
/*! Display a Windows message box.
    ...
    @param zsTitle Title of the message box. */
void AC IC_MsgBox( char * szText, char* szTitle ); ///<Display a windows message box.
...
// Random notes:
// -*- VCDID_AutoRoi is JUST THE ROI USED FOR AUTO GAIN/EXPOSURE
...
/*! Show the VCDProperty dialog.
    ...
    @retval IC_NO_DEVICE No video capture device selected.
    @retval IC_NO_HANDLE NULL pointer. */
...
/*! Enumerate the available properties of a video capture device.
    ...
    @retval IC_SUCCESS No error otherwise an error occurred, e.g. no device selected. */
...
/*! Enumerate the available elements of a video capture device and a property.
    ...
    @retval IC_SUCCESS No error otherwise an error occurred, e.g. no device selected.
    @retval IC_PROPERTY_ITEM_NOT_AVAILABLE The passed property in Property is not available. */
...
/*! This returns the range of a property.  For a list of properties and
    ...
    if( IC_InitLibrary(0) )
    {
        ...
        if( hGrabber )
        {
            ...
            if( Result == IC_SUCCESS )
                printf("Exposure Auto Reference Min %d, Max %d\n", Min, Max);
...
/*! Return the current set string of a MapString interface
    ...
    @param Property  The name of the property, e.g. "Strobe"
    @param Element  The type of the interface, e.g. "Mode"
    @param String     pointer to a char*. Size should be at least 50. There is no check! This contains the result.
...
//////////////////////////////////////////////////////////////////
/*! Query number of available frame filters

    @retval The count of found frame filters. */
int AC IC_GetAvailableFrameFilterCount();

//////////////////////////////////////////////////////////////////
/*! Query a list of frame filters

    @param szFilterList A two dimensional char array that receives the list of found frame filters
...
///////////////////////////////////////////////////////////////
/*! Delete the memory allocated by the HFRAMEFILTER structure. Please remove
    the frame filter from the HGrabber, before deleting it.
...
/*! Set an int parameter value of a frame filter
    ...
    @retval IC_SUCCESS    Success
    @retval IC_PROPERTY_ITEM_NOT_AVAILABLE the parameter given by ParameterName does not exist
...
////////////////////////////////////////////////////////////////////////////
/*! Callback type definition for the CODEC enumeration callback called by
    IC_enumCodecs
    @retval 1 : Terminate the enumeration, 0 continue enumerating
*/
...
/*! Enumerate all installed CODECs. It calls the callback function passed by
    ...
    @param cb callback function of type ENUMCODECCB
...
/*! Assigns the selected CODEC to the Grabber. AVI Capture is prepared. Image
    ...
    After doing so, a call to IC_Startlive() starts AVI Capture and IC_Stoplive stops it,
