...
CU_111_VALID_TARGETS = {
    ...
    "Cu_def L(2-4)": [False, False, True, True, False],
}
...
class ValidTargets:
    @pytest.mark.parametrize("target_str_mask", CU_111_VALID_TARGETS.items(),
                             ids=CU_111_VALID_TARGETS.keys())
    @case(tags="cu_111")
    def case_cu_111_exact(self, cu_111_fixed_l_max_state_after_init, target_str_mask):
...
class InvalidTargets:
    @pytest.mark.parametrize("target_str", CU_111_INVALID_TARGETS,
                             ids=CU_111_INVALID_TARGETS)
    @case(tags="cu_111")
    def case_cu_111_exact(self, cu_111_fixed_l_max_state_after_init, target_str):
        slab, _ = cu_111_fixed_l_max_state_after_init
        base_scatterers = get_base_scatterers(slab)
        return base_scatterers, target_str

@parametrize_with_cases("base_scatterers,target_str,expected_mask",
                        cases=ValidTargets)
