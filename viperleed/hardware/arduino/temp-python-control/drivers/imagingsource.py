...
class ImagingSourceDMKCamera(Camera):
    ...
    def fail(self, funct, *args, **kwargs):
        """
        Runs self.camera_lib.funct(*args, **kwargs) and returns True if the
        action failed
        """
    ...
    def initialize(self, camera_config):
        ...
        # try connecting
        if self.fail('openVideoCaptureDevice', camera_config['device_name']):
        ...
        if camera_config['set_roi'] == 'True':
                self.set_roi(camera_config['roi_x'], camera_config['roi_y'])
        
        # set binning
        self.set_binning(camera_config['bin_factor'])
        
        # set exposure, gain, number of frames, frame rate,image path, 
        # nameprefix
        self.set_exposure(camera_config['exposure_time'])
        self.set_gain(camera_config['gain'])
        self.set_n_frames(camera_config['n_frames'])
        self.set_framerate(camera_config['frame_rate'])
        self.set_imagepath(camera_config['image_path'],
                           camera_config['file_name'])
        
        self.set_callback()
        self.set_continuous_mode(camera_config['live_mode'])
        self.enable_trigger(camera_config['live_mode'])
