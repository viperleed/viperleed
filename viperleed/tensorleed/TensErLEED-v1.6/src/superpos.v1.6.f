C  Tensor LEED intensity calculation v1.2
C  VB, 13.04.00
C  for use with lib.superpos.f v1.2
C
C  as described in 
C
...
C
     ...
     1                  IFORM(IFIL)
C           write(3,*) "Complete"
           READ (5,10) (IFNUM(I,IFIL),I=1,IFVAR(IFIL))
...
C
      DO 510 IFIL=1,NFILES
           CALL INRINT(10+IFIL,IFORM(IFIL),TE,VV,VO,VPI,NT0,
...
C
      BKZ = CMPLX(2.0*E-2.0*VO-BK2*BK2-BK3*BK3,-2.0*VPI)
      BKZ = CSQRT(BKZ)
C
...
C
  ...
  552 FORMAT (1F7.2,1F7.4,4E14.5,/,100(5E14.5,/))
C
C     write documentation file (if first energy .eq. 1)
      IF (FE.eq.1.) THEN
         WRITE(3,560) IOUT
  560    FORMAT(' Geometry number ',I3,':')
         WRITE(3,561) ICONC
 561     FORMAT(' Concentration no. ',I3)
         DO 570 I=1,NFILES
            WRITE(3,565) I,IFNUM(ICNT(I),I)
  565       FORMAT(' File ',I3,' Variation no : ',I3)
  570    CONTINUE
