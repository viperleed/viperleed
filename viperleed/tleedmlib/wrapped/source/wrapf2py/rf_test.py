...
def plot_csv(prep_exp, prep_theo, exp_corr, theo_corr, beam):
    ...
    i_ax.plot(
        out_grid, prep_theo[2][:, beam] / (np.max(prep_theo[2])), label="Theoretical"
    )
    i_ax.plot(
        out_grid, prep_exp[2][:, beam] / (np.max(prep_exp[2])), label="Experimental"
    )
    i_ax.legend()
...
def prepare(data, in_grid, out_grid, deg, which_r, v0i):
>>>>>>> Stashed changes
>>>>>>> Stashed changes
...
=======
    ...
    x = out_grid[prep_exp[0][beam] : prep_exp[0][beam] + prep_exp[1][beam]]
    y_func_test = prep_exp[-1][
        prep_exp[0][beam] : prep_exp[0][beam] + prep_exp[1][beam], beam
    ]
>>>>>>> Stashed changes
>>>>>>> Stashed changes
...
=======
    ...
    prep_exp = prepare(exp, grid_exp, out_grid, deg, v0i)
    ...
    plt.plot(
        x,
        y_func_test,
        label=f"Y func original data grid",
        color="orange",
        ls="",
        marker="x",
        ms=5,
...
=======
        ...
        for deg in (
            ...
            knots, coeffs, ierr = intpol.single_calc_spline(
                x, y_func_test, deg, n_knots, nt, LHS_rows
            )
...
=======
=======
            y_func_out = intpol.single_interpolate_coeffs_to_grid(
                knots, coeffs, deg, x_new, 0
            )

            plt.plot(
                x_new,
                y_func_out,
                label=f"Re-interpolated deg={deg}, Y_supersample_rate={i}",
                ls="--",
            )

    plt.xlim(
        out_grid[prep_exp[0][beam]], out_grid[prep_exp[0][beam] + prep_exp[1][beam]]
    )
    plt.ylim([-1, 1])
>>>>>>> Stashed changes
>>>>>>> Stashed changes
...
prep_exp = prepare(exp, grid_exp, out_grid, 5, v0i=v0i)
print(theo)
print(grid_theo)
prep_theo = prepare(theo, grid_theo, out_grid, 5, v0i=v0i)
