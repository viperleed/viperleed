...
70    FORMAT(9H*BEAM NO.,1I5,10H IN EXP. (,4A4,20H), WHICH IS BEAM NO.,
     11I5,12H IN THEORY (,2F12.4,1H))
...
C      WRITE(6,705)IBE                                                     .
705   FORMAT(48H0 **** NO OVERLAP THEOR./EXP. IN (EXP.) BEAM NO.,1I5)
...
C
     ...
     +                 IGEN,WHICHG,WHICHR,PMISCH,DMISCH)
      ...
      text1 = 'IND ||  R-Av  |  R-Int | R-Frac |   V0r   || Proz | '
      text2 = '                                                    '
      text3 = '                                          ||'
      text = text1
...
cas     +IFILE=1,NFIL(IPLACE)),('C',IPLACE,IPLACE,',')),IPLACE=1,MNPLACES)
 ...
 5559 FORMAT(I3,A4,F6.4,A3,F6.4,A3,F6.4,A3,F7.3,A4,i3,a4,5000(I3,A1))
 5560 format(a44,i4,a4,5000(I3,A1))
...
C --- Schleife ueber alle Domaenen
     ...
     +                 "delta amplitude storage files required for"
          ...
          write(6,'(a100)') text
          write(6,'(I5)') NSURF(IDOM,IPLACE)
          write(6,'(I5)') FILREL(IDOM,IPLACE)
          write(6,'(I5)') NFIL(IDOM,IPLACE)
...
C --- Pro Domaene ein zusaetzlicher Parameter
 ...
 2010 continue
      ...
      IF (STAFLA.EQ.1) THEN
        DO 1370 IPOP = 1, NPS
          READ(21,'(5000I4)') (PARIND(IPARAM,IPOP),IPARAM=1,NPRMK)
          write(6,'(5000I4)') (PARIND(IPARAM,IPOP),IPARAM=1,NPRMK)
 1370   CONTINUE
      ENDIF

cvb      write(6,*) 'Inhalt von VARST():'
cvb      write(6,'(5000i4)') (VARST(i), i = 1, PNUM)

cvb      write(6,*) 'Inhalt von NPRAS():'
cvb      write(6,'(5000i4)') (NPRAS(IDOM), IDOM = 1, NDOM)
...
C  READ AND PRINT DESCRIPTION OF EXPERIMENT
      ...
      READ(5,30)(NBEA(I),I=1,NBED)
      WRITE(6,40)NBED
C     WRITE(8,40)NBED
40    FORMAT(1H*,I4,25H EXP. BEAMS TO BE READ IN)                         040481
      WRITE(6,50)(NBEA(I),I=1,NBED)
C     WRITE(8,50)(NBEA(I),I=1,NBED)
50    FORMAT(31H*AVERAGING SCHEME OF EXP. BEAMS,400(25I4,/))
C  READ INPUT FORMAT OF EXP. INTENSITIES
      READ(5,'(A80)') FMT
      DO 90 IB=1,NBED
C  READ AND PRINT NAME OF THIS BEAM
      READ(5,10)(BENAME(I,IB),I=1,5)
10    FORMAT(19A4)
      WRITE(6,70)IB,(BENAME(I,IB),I=1,5)
C     WRITE(8,70)IB,(BENAME(I,IB),I=1,5)
70    FORMAT(11H*EXP. BEAM ,1I5,2H (,5A4,1H))
...
C  READ (AND MAYBE PRINT) EXP. INTENSITIES
      READ(5,FMT)(EE(IB,IE),AE(IB,IE),IE=1,N)
      IF (IPR.LT.1) GO TO 82
      WRITE(6,80)(EE(IB,IE),AE(IB,IE),IE=1,N)
80    FORMAT(31H* EXP. ENERGIES AND INTENSITIES,
...
C  begin readin from stdin
      ...
      READ(5,'(I4)')   ISMOTH
      READ(5,'(I4)')   EOT
      READ(5,'(2I4)')  NBTD,NBED

      WRITE(6,8)NBTD,NBED                                               040481
8     FORMAT(13H*NBTD,NBED = ,2I4)                                      040481

      READ(5,'(25I4)') (KAV(J),J=1,NBTD)

      WRITE(6,5)(KAV(J),J=1,NBTD)

5     FORMAT(33H*AVERAGING SCHEME OF THEOR. BEAMS,400(25I4,/))

C Values for averaging of beams in integer and half order beams
      READ(5,*) 
      READ(5,'(25I4)') (MITTEL(I),I=1,NBED)

C beam order IBP, beam weights WR

      READ(5,*)
      READ(5,'(25I4)')   (IBP(I),I=1,NBED)
      READ(5,'(25F4.1)') (WB(I),I=1,NBED)
...
60    FORMAT(48H*EXP. ENERG. AND INTENS. AFTER SMOOTHING IN BEAM,1I5,
     1/,1000(5(1F7.2,1E13.4,3X),/))
50    CONTINUE
70    RETURN
      END
...
80    FORMAT(40H*INTENSITIES AFTER INTERPOLATION IN BEAM,1I5,
     1/,1000(5(1F7.2,1E13.4,3X),/))
70    CONTINUE
90    RETURN
      END
...
110   FORMAT(48H*EXP. ENERG. AND INTENS. AFTER AVERAGING IN BEAM,1I5,/,
     11000(5(1F7.2,1E13.4,3X),/))
120   RETURN
      END
C------------------------------------------------------------------------
...
C
  ...
  103 FORMAT(I5,E13.4)
  104 FORMAT(A27)
  105 FORMAT(F7.2,F7.4,4E14.5,/,2000(5E14.5,/))
  106 FORMAT(8F10.3)
C
C
C BEGIN READ IN PART
      ...
      ENDIF
      DO 210 IB=1,NBED   
         READ(5,101) INN,(BENAME(I,IB),I=1,5)
...
C
      DO 230 J=1,NDATA
         READ (5,105,END=250) EEH(J),DUMMY,(AE(K,J),K=1,NBED)
...
C  Read in headers of all files
     ...
     +                              (TRAR2(I),I=1,2)
            READ(CNTFIL,'(3I5)') INT0,NATOMS,NCSTEP
...
C  if energy is within range save it in array ESMK
            ...
            EeV = (EMK(IDATT)-VV(IDATT))*27.211386
...
C  check validity of read energy
              ...
              IF (CNTFIL.eq.11) THEN
                ...
                EMK0(IDATT) = EMK(IDATT)

              ELSE

                CHECK = ABS(EMK(IDATT)-EMK0(IDATT))

                IF (CHECK.gt.1.E-9) THEN
...
C  to allow for variable arrays cause f77 cannot handle anything intelligently.
      ...
      Subroutine ReadDels(CNTFIL,NDOM,IDOM,E,VPI,VV,XISTMK,DELMK,
...
C  IFNUM in those files. 'Concentration' of different files can be varied here.
     ...
     +                  DATTNO,EMK,VV,VPI,THETA,FI,PQFEX,RAR1,RAR2,
     +                  XISTMK,DELMK,ATSAS)
...
C  no of energies to be considered, energy values, inner potential
      ...
      REAL EMK,VV,VPI
      DIMENSION EMK(NDATT),VV(NDATT),VPI(NDATT)
...
C   and vacuum)
        ...
        BKZ=CMPLX(2.0*E-BK2*BK2-BK3*BK3,-2.0*VPI(IDATT))
        BKZ=CSQRT(BKZ)
...
C  files.
     ...
     +                    VARST,NDATT,DATTNO)
      ...
      IF (PNUM.NE.MNPRMK) THEN
         ...
         write(8,'("PNUM:",I5,"MNPRMK:",I5)') PNUM, MNPRMK
