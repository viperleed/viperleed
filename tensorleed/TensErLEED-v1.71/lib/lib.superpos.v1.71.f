...
C  v1.7, Orig. from VB 13.04.00
C  for use with superpos.f v1.7
C
C  as described in 
C
...
C**********************************************************************************
C
C  Please read the comment in superpos.f, v1.7 .
C
C-----------------------------------------------------------------------
...
C
      SUBROUTINE INRINT(IFILE,IFORM,E,VV,VPI,NT0,NCSTEP,XIST,DELWV)
C
      COMPLEX XIST(NT0), DELWV(NCSTEP,NT0)                              11/11/90
C
      IF (IFORM.EQ.0) THEN
         READ (IFILE,ERR=200,END=200) E,VPI,VV,XIST,DELWV
      ELSE
         READ (IFILE,100,ERR=200,END=200) E,VPI,VV
