...
C  OUTPUT REDUCED FOR ERROR CURVE CALCULATIONS   L.HAMMER JAN. 2016
...
C  NV0   : number of variations of the real part of the inner potential 
...
C  read in any non-default values of the NL1 variables (as supplied by user)
 ...
 3    FORMAT(13H0NBTD,NBED = ,2I4)                                       040481
...
C  read beam labelling
 ...
 9    FORMAT(7HMITTEL(,I3,26H) MUST BE EITHER 0, 1 OR 2,/,
     *       36H   *****   PLEASE CORRECT   *****   )
...
C  perform domain averaging and check for too high theoretical intensities
 ...
 28       FORMAT(1H ,//,29H MAX INTENSITY IN THEOR BEAM ,1I3,            121280
     *           12H IS SUSPECT(,1E13.5,25H)- **** STOP PROGRAM ****)    121280
      ...
      WRITE(7,29)
 29   FORMAT(1X,/,'R-Factor',7X,'D12',5X,'V0R',5X,'RAV')                                   201103
...
C  ascertain corresponding theoretical beam and skip beams if necessary
 ...
 66     FORMAT(12H0EXP BEAM NO,1I3,10H SKIPPED (,5A4,1H))
        GO TO 100
      ENDIF
...
C  skip beam if no overlap between experiment and theory occurs
      ...
      IF (EET(IBE).EQ.0.) THEN
        WRITE(6,75) IBE
C        WRITE(7,80) (BENAME(I,IBE),I=1,4),IBE,D12,V0R,EMIN,EMAX,0.,0.
 75     FORMAT(48H0 **** NO OVERLAP THEOR./EXP. IN (EXP.) BEAM NO.,1I3)
 80     FORMAT(4A4,I5,1F8.4,4F8.2,1F8.4)
        GO TO 100
      ENDIF

C  find MAX(ABS(derivative of exp intensitiese)) for Zanazzi-Jona R-factor
...
C      WRITE(7,80) (BENAME(I,IBE),I=1,4),IBE,D12,V0R,EMIN,EMAX,
C     *               EET(IBE),RAVB(IBE)
...
C        WRITE(7,125) -1,D12,V0R,EMIN,EMAX,ERANGM(1),RAVM(1)              141290
      ENDIF

      IF (NFRAC.NE.0) THEN
        WRITE(6,120) V0,ERANGM(2),(ARM(I,2),I=1,3),RAVM(2)
C        WRITE(7,126) -1,D12,V0R,EMIN,EMAX,ERANGM(2),RAVM(2)              141290
      ENDIF
  
      WRITE(6,120) V0,ERANG    ,(AR(I),I=1,3), RAV
C      WRITE(7,127)  0,D12,V0R,EMIN,EMAX,ERANG,RAV                        141290

      IF (NFRAC.NE.0.AND.NINT.NE.0) THEN
        WRITE(6,128) CIFE,CIFT
      ENDIF

 125  FORMAT('AV.-INT         ',I5,F8.4,4F8.2,F8.4,'  <---')             260187
 126  FORMAT('AV.-FRAC ORD    ',I5,F8.4,4F8.2,F8.4,'  <---')             260187
 127  FORMAT('AVERAGE    'F8.4,3F8.2,'  ',F8.2,' ',F8.4)
 128  FORMAT('RELATION FRAC TO INT BEAM INTENSITIES:',F6.3,
     *       ' FOR EXP DATA,',F6.3,' FOR THEOR DATA')
 129  FORMAT(' ')
 130  FORMAT('AVERAGE    'F8.4,F8.2,' ',F8.4)
 131  FORMAT(F8.4)
...
C      WRITE(7,175) NSB,VALUES(NSB),BV0,BRAV
      IF (NFRAC.NE.0.AND.NINT.NE.0) THEN
C        WRITE(7,128) BCIFE,BCIFT
      ENDIF
