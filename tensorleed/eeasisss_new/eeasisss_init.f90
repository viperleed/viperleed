...
nteger,parameter :: nshell=8
nteger :: nieq,nlat
nteger,allocatable :: neq(:),ieq(:)
...
nxx     = max nx(:).
real(dp) :: rmtovl
eal(dp),allocatable :: rx(:,:),dx(:),rmin(:),rmax(:),rmt(:),rmtovl(:)
nteger,allocatable  :: nx(:)
nteger :: nxx
unit cell,
...
nteger,allocatable ::  na(:,:),ia(:,:),ncon(:)
eal(dp),allocatable :: rk(:,:),ad(:,:)
eal(dp) :: rc(3,3),volUC,volXC
ontains
!----------------------------------------------------------------------
...
==============================================================================
odule enrgy
se param,only : dp,z,lmax,eev1,eev2,emesh,linux,windows
se space,only : rx,dx,nx,rmt,ad,ia,neq,nlat
mplicit none
Vnist = initial atomic potential corresponding to rho,
      = tends to zero at radius infty, Ref. Eric Shirley, NIST,
eal(dp) :: Vcry0
eal(dp),allocatable :: atrho4pir2(:,:),rho(:,:),rs(:,:),&
...
nteger  :: ne,m,kappa,iatom
eal(dp) :: Einc
eal(dp),allocatable :: eev(:),emv0(:),v0ev(:),xcfac(:)
...
eal(dp),dimension(92),parameter :: anu=(/&
...
39.d0,140.d0,141.d0,144.d0,145.d0,150.d0,152.d0,157.d0,159.d0,163.d0,165.d0,167.d0,169.d0,173.d0,&
75.d0,178.d0,181.d0,184.d0,186.d0,190.d0,192.d0,195.d0,197.d0,201.d0,204.d0,207.d0,209.d0,209.d0,&
10.d0,222.d0,223.d0,226.d0,227.d0,232.d0,231.d0,238.d0/)
ontains
!----------------------------------------------------------------------
...
nteger,parameter :: nsr=17,nsp=151
eal(dp),dimension(nsr),parameter :: sr=(/&
.00d0,0.01d0,0.02d0,0.05d0,0.10d0,0.20d0,0.30d0,0.40d0,0.50d0,0.70d0,1.00d0,1.50d0,2.00d0,3.00d0,&
.00d0,5.00d0,6.00d0/)
eal(dp),dimension(nsp),parameter :: sp=(/&
.00d0,0.02d0,0.04d0,0.06d0,0.08d0,0.10d0,0.12d0,0.14d0,0.16d0,0.18d0,0.20d0,0.22d0,0.24d0,0.26d0,&
.28d0,0.30d0,0.32d0,0.34d0,0.36d0,0.38d0,0.40d0,0.42d0,0.44d0,0.46d0,0.48d0,0.50d0,0.52d0,0.54d0,&
.56d0,0.58d0,0.60d0,0.62d0,0.64d0,0.66d0,0.68d0,0.70d0,0.72d0,0.74d0,0.76d0,0.78d0,0.80d0,0.82d0,&
.84d0,0.86d0,0.88d0,0.90d0,0.92d0,0.94d0,0.96d0,0.98d0,1.00d0,1.02d0,1.04d0,1.06d0,1.08d0,1.10d0,&
.12d0,1.14d0,1.16d0,1.18d0,1.20d0,1.22d0,1.24d0,1.26d0,1.28d0,1.30d0,1.32d0,1.34d0,1.36d0,1.38d0,&
.40d0,1.42d0,1.44d0,1.46d0,1.48d0,1.50d0,1.52d0,1.54d0,1.56d0,1.58d0,1.60d0,1.62d0,1.64d0,1.66d0,&
.68d0,1.70d0,1.72d0,1.74d0,1.76d0,1.78d0,1.80d0,1.82d0,1.84d0,1.86d0,1.88d0,1.90d0,1.92d0,1.94d0,&
.96d0,1.98d0,2.00d0,2.02d0,2.04d0,2.06d0,2.08d0,2.10d0,2.12d0,2.14d0,2.16d0,2.18d0,2.20d0,2.22d0,&
.24d0,2.26d0,2.28d0,2.30d0,2.32d0,2.34d0,2.36d0,2.38d0,2.40d0,2.42d0,2.44d0,2.46d0,2.48d0,2.50d0,&
.52d0,2.54d0,2.56d0,2.58d0,2.60d0,2.62d0,2.64d0,2.66d0,2.68d0,2.70d0,2.72d0,2.74d0,2.76d0,2.78d0,&
.80d0,2.82d0,2.84d0,2.86d0,2.88d0,2.90d0,2.92d0,2.94d0,2.96d0,2.98d0,3.00d0/)

eal(dp),dimension(nsp,nsr) :: sdat
eal(dp),dimension(nsp*nsr),parameter :: sdat1=(/&
...
nd module DifferentialEvolution
