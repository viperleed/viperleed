C  Tensor LEED subroutine library for delta amplitude calculation v1.7
C  VB, 13.04.00  modified by  LH 23.04.21 (for read in of tensors with variable lmax)
C  for use with delta.f v1.7
C
C  as described in 
C
...
C**********************************************************************************
C
C  Please read the comments in delta.f, v1.7
C
...
C
      SUBROUTINE INDATA(IFILE,IFORM,E,L1,CAF,NT0,XIST,VPIS,VPIO,VV,
     1 LMDAT)
C
      REAL E,VPIS,VPIO,VV
      ...
      IF (IFORM.EQ.0) THEN
      READ(IFILE,ERR=100,END=200)E,VPIS,VPIO,VV,L1DAT,CAF,XIST
      ELSE
      READ(IFILE,20,ERR=100,END=200)E,VPIS,VPIO,VV
...
C
      SUBROUTINE OUTRINT(IFILE,IFORM,E,VV,VPI,NT0,NCSTEP,
     1                   XIST,DELWV)
C
      COMPLEX XIST(NT0),DELWV(NCSTEP,NT0)
C
      IF (IFORM.EQ.0) THEN
      WRITE (IFILE) E,VPI,VV,XIST,DELWV
      ELSE
      WRITE (IFILE,100) E,VPI,VV
...
C  variable dimensions
      ...
      integer NT0,NPSI,NEL,NATOMS,NCSTEP,NDEB,PSFMT
...
C  Y- AND Z-AXES (X-AXIS IS PERPENDICULAR TO SURFACE)(ANGSTROM)
      ...
      READ(5,'(2F9.4)')(ARA1(I),I=1,2)
      READ(5,'(2F9.4)')(ARA2(I),I=1,2)

c      WRITE(6,'(2F9.4)')(ARA1(I),I=1,2)
c      WRITE(6,'2F9.4)')(ARA2(I),I=1,2)
...
C  info on lateral periodicity: superlattice 
      ...
      READ(5,'(2F9.4)')(ARB1(I),I=1,2)
      READ(5,'(2F9.4)')(ARB2(I),I=1,2)
c      WRITE(6,'(2F9.4)')(ARB1(I),I=1,2)
c      WRITE(6,'(2F9.4)')(ARB2(I),I=1,2)
      DO 467 I=1,2
      ARB1(I)=ARB1(I)/BOHR
...
C  incidence angles theta, fi
      ...
      READ (5,'(2F9.4)')  THETA, FI

C s.w. conversion in radians, 02.02.00
      THETA=THETA*PI/180.0
      FI=FI*PI/180.0

C  PSFMT indicates the format of the phaseshift file
...
C        read (5,*)
C        read (5,*)
C        read (5,'(3F7.4)') (CUNDISP(IATOM,IPOS), IPOS=1,3)

        do IPOS = 1,3
          CUNDISP(IATOM,IPOS) = 0.0000
        enddo
