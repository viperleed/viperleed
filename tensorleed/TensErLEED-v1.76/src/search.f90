...
C  quot is used for energy-dependent inner potential V0R=const+E/quot
      ...
      REAL VPI,VV
      DIMENSION VPI(MNDATT),VV(MNDATT)
...
C  write current parameter values to control file
      ...
      do 2100 IPOP = 1, NPS
        ...
        IF (IGEN.eq.1) THEN
        write(8,'(5000I4)') (PARIND(IPARAM, IPOP), IPARAM = 1, NPRMK)
        ELSE
        write(8,'(5000I4)') (PARHELP(IPARAM, IPOP), IPARAM = 1, NPRMK)
        END IF

      ! flush I/O buffer for control.chem
      ! this prevents OS-level I/O race conditions
      call flush(8)
