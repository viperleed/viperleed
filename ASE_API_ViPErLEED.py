...
def refcalc_for_ase_structure(
    ...
    V0i : float
        Imaginary part of the inner potential as read from "PARAMETERS". This is returned here since it is an input parameter for the R-factor calculation.
    ...
    # Get temporary parameters object: dummy mostly but needed for getSurfaceAtoms
    rp = readPARAMETERS(os.path.join(exec_path, parameters_name))
    V0i = float(rp.V0_IMAG)  # typecast in case it's a string
...
def run_rfactor_from_csv(
    ...
    for i, (is_content, file) in enumerate(zip(beams_file_is_content, beams_files)):
        if is_content:
            ...
            beams_tmp[i] = readOUTBEAMS(filename=None, sep=",", file_StringIO=io_file)
        else:
