...
def main():
    ...
    parser.add_argument("-c", "--cont", help="overwrite POSCAR and VIBROCC "
                "with POSCAR_OUT and VIBROCC_OUT from the OUT folder, if "
                "present", action="store_true")
    parser.add_argument("-n", "--name", help="defines a string to be appended "
                "to the name of the history folder that is created, and is "
                "logged in history.info", type=str)
    args = parser.parse_args()
    # make list of stuff to move
    tomove = [d for d in os.listdir() if os.path.isdir(d) and
                (d == "OUT" or d == "AUX")]
    tomove.extend([f for f in os.listdir() if os.path.isfile(f) and
                 f.endswith(".log")])
    ...
    if args.cont == True:
        if os.path.isdir("OUT"):
            fout = sorted([f for f in os.listdir("OUT") if 
