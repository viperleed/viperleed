...
\textbf{Communication protocol}: Each communication message between PC and Arduino is composed of \texttt{MSG\_START} + 1~byte for length of message that follows + \texttt{message} + \texttt{MSG\_END}. With the exception of error messages sent back from the Arduino to the PC, each byte in \texttt{message} as well as in the length byte (TRUE?? If it is also encoded, we are not decoding it. Otherwise we are limited to \texttt{MSG\_END}~$-1$  bytes) is encoded as follows: if the ASCII representation of the byte would be larger or equal (??) than a \texttt{MSG\_SPECIAL\_BYTE}, the byte is transformed into 2 bytes, the first one is \texttt{MSG\_SPECIAL\_BYTE}, the second is \texttt{byte} $-$ \texttt{MSG\_SPECIAL\_BYTE}. Currently, error messages are a mess.
...
\begin{center}
\begin{tabular}{lcp{.32\textwidth}lc}\toprule
...
\texttt{PC\_SET\_VOLTAGE} & 4          & dacValue MBS, dacValue LSB, dacSettlingTime MSB, dacSettlingTime LSB & None & ---\\
...
\item Error messages from several possible states. Right now the error messages are a mess. Possible error codes are
	...
	\texttt{ERROR\_MSG\_UNKNOWN}        & 4     & The PC sent an unknown command, i.e. not one of those in paragraph \textit{From PC to Arduino} at the beginning of this Section.\\
	\texttt{ERROR\_MSG\_DATA\_INVALID}  & 5     & The message was understood, but some of the data passed is inappropriate.\\
	\texttt{ERROR\_REQUEST\_TOO\_EARLY} & 6     & The message was understood, but the request cannot be fulfilled because it requires information that needs another command to come before this.\\
...
\texttt{STATE\_INITIAL\_CALIBRATION} & \texttt{PC\_CALIBRATION}   & \texttt{initialCalibration()} & \texttt{STATE\_IDLE}\\
