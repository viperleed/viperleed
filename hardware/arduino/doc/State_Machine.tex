...
\begin{itemize}
\item 6 bytes [encoded] containing the information on the current firmware version (first 4 bytes) and hardware settings (last 2), as a reply to \texttt{PC\_CONFIGURATION}. The firmware version comes as 4 digits, i.e. (MAJOR MSB, MAJOR LSB, MINOR MSB, MINOR LSB), each of which can go from zero to 9 (max `9999', i.e. v99.99). The bits in the hardware information have following meaning:
	  \begin{center}
...
\item Error messages from several possible states. Each error causes two messages to be sent: The first one is the (encoded) \texttt{PC\_ERROR} byte, the second one is a 2-byte data message, where the first byte corresponds to the state in which the error occurred, while the second one is one of this error codes:
