...
import sys
import numpy as np
if os.path.join(os.path.dirname(__file__), 'Camera_libraries') not in sys.path: 
    sys.path.append(os.path.join(os.path.dirname(__file__), 'Camera_libraries'))
...
# ############################################################################
    ...
    IsCameraPropertyAvailable = __tisgrabber.IC_IsCameraPropertyAvailable
    IsCameraPropertyAvailable.restype = C.c_int
    IsCameraPropertyAvailable.argtypes= (GrabberHandlePtr,
                                 C.c_char_p,)
    ...
    GetFrameRate =  __tisgrabber.IC_GetFrameRate
    GetFrameRate.restype = C.c_float
    GetFrameRate.argtypes = (GrabberHandlePtr,)
    
    GetPropertyValueRange = __tisgrabber.IC_GetPropertyValueRange
    GetPropertyValueRange.restype = C.c_int
    GetPropertyValueRange.argtypes= (GrabberHandlePtr,
                                     C.c_char_p,
                                     C.c_char_p,
                                     C.POINTER(C.c_long),
                                     C.POINTER(C.c_long))
...
# ############################################################################
    EnableAutoCameraProperty = __tisgrabber.IC_EnableAutoCameraProperty
    EnableAutoCameraProperty.restype = C.c_int
    EnableAutoCameraProperty.argtypes = (GrabberHandlePtr,
                            C.c_int,
                            C.c_int)
...
# ############################################################################
    ...
    SetFrameReadyCallback.argtypes = [GrabberHandlePtr, FRAMEREADYCALLBACK,
                                      C.py_object]
...
class TIS_CAM(object):
        ...
        def SetFormat(self, Format):
            ...
            '''
            return TIS_GrabberDLL.SetFormat(self._handle, Format.value)
        ...
        def GetFrameRate(self):
            Value = C.c_long()
            Value =  TIS_GrabberDLL.GetFrameRate(self._handle)
            
            return Value
        
        def IsCameraPropertyAvailable(self, Property):
            
            error = TIS_GrabberDLL.IsCameraPropertyAvailable(self._handle,
                                                       self.s(Property))
            return error
        ...
        def GetPropertyValueRange(self, Property, Element, Value_min, Value_max):
            ...
            error = TIS_GrabberDLL.GetPropertyValueRange(self._handle, 
                                                         self.s(Property), 
                                                         self.s(Element),
                                                         minValue,
                                                         maxValue,)
        ...
        def enableAutoCameraProperty(self, property, onoff):
            return TIS_GrabberDLL.EnableAutoCameraProperty(self._handle,property,onoff)
        ...
        def IsTriggerAvailable(self):
            error = TIS_GrabberDLL.IsTriggerAvailable(self._handle)
            
            return error
        
        def EnableTrigger(self, onoff):
            error = TIS_GrabberDLL.EnableTrigger(self._handle, onoff)
            
            return error
        
        def SoftwareTrigger(self):
            error = TIS_GrabberDLL.SoftwareTrigger(self._handle)
            
            return error
        
        def ResetProperties(self):
            
            error = TIS_GrabberDLL.ResetProperties(self._handle)
            
            return error
#        def GetTriggerModes(self, ModeList, Size):
#            error = TIS_GrabberDLL.GetTriggerModes(self._handle,
#                                                   ModeList, 
#                                                   Size)
#            return error
