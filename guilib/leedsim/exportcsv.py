...
========================================
*** module guilib.leedsim.export_csv ***
...
def export_pattern_csv(fnames, leeds, **kwargs):
    ...
    if isinstance(fnames, (list, tuple, np.ndarray)):
        ...
        if len(fnames) != len(leeds):
            raise ValueError("export_csv: not enough file paths given for "
    ...
    if len(all_domains) != len(leeds):
        raise ValueError("export_csv: The number of domains definition indices "
    ...
    if any(not isinstance(leed, (dict, gl.LEEDPattern)) for leed in leeds):
        raise TypeError("export_csv: each of the LEED parameters passed must "
    ...
    for i, leed in enumerate(leeds):
        ...
        if not np.allclose(leed.bulkR.basis, leeds[0].bulkR.basis):
            raise ValueError("export_csv: Incompatible bulk bases found among "
                             "the LEED patterns")

    # Now go for the actual code
    all_lines = []
    for leed, domains, fname in zip(leeds, all_domains, fnames):
...
def _format_beams_(leed, **kwargs):
    ...
    if not isinstance(leed, gl.LEEDPattern):
        raise TypeError("export_csv: leed must be a "
    ...
    for fract in fractions:
        ...
        hk.append(beam)
        g = np.dot(beam, leed.bulkR.basis)
        gg.append(g)
    ...
    # get the maximum of all lengths, that will determine the formatting
...
def _format_header_(lengths, leed, **kwargs):
    """
    ...
    if any((key not in lengths.keys()) for key in needed_keys):
        raise ValueError("export_csv: some keys are missing from the required "
                         "parameter 'lengths'.")
    ...
    source = kwargs.get('source', None)
    name = kwargs.get('name', None)
    export_domains = kwargs.get('domains', range(leed.nDoms))
    ...
    header.append(f"# Max. LEED Energy: {leed.maxEnergy:.1f} eV")

    for (txt, lattR) in zip(['Bulk', 'Surface'],
                            [leed.bulkR, leed.surfR]):
    ...
    matrices = leed.domSuperlattices[export_domains]

    header.append(f"#\n# Exporting beams from {len(matrices)} domain(s) "
                  f"out of {leed.nDoms} symmetry-equivalent ones")

    basis = leed.bulkR.reciprocal_basis()
