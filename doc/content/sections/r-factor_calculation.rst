...
(\ :ref:`search section<sec_search>`) minimizes the |R factor| between
the calculated and experimental |IV| curves.
...
-  :math:`R_\mathrm{P}` compares the logarithmic derivatives of the |IV|
   data, using a fix for the divergence of the logarithm when the intensity
   approaches zero.
-  :math:`R_2` is based on the mean-square difference of the (scaled) |IV|
   curves.

.. note::
    Using :math:`R_\mathrm{P}` is the **default setting** and highly encouraged
    since tests have shown that it leads to better results than :math:`R_2`
    :cite:p:`spornAccuracyQuantitativeLEED1998`.
...
The Pendry |R factor| :math:`R_\mathrm{P}` can have values between 0 and 2 and
is defined as
...
indicate anti-correlation. The best values of :math:`R_\mathrm{P}` obtained by
...
incorrect structure model. |R factor|\ s for more open surfaces, such as
...
values of :math:`R_\mathrm{P}`. Some LEED programs apply such a smoothing; in
this case lower |R factor|\ s than those obtained with ViPErLEED will be
...
optimization and calculating the |R factor|. We highly recommend to
smooth experimental data beforehand using the |IV|-curve editor in the
:ref:`ViPErLEED Spot Tracker<spot_tracker>`. The :ref:`EXPBEAMS.csv<EXPBEAMS>`
