...
Ref. :cite:p:`ferstlStructureOrderingOxygen2016`. The experimental
...
As reference structure, we use a :ref:`POSCAR file<poscar>` containing an
Ir(100) surface that was hand made from a bulk structure. An additional
adsorbed oxygen atom is placed at a bridge site, such that each surface
iridium atom binds to one oxygen atom.
...
to set up a :ref:`DISPLACEMENTS file<displacements>`. Since this is the first
run starting from a totally unrelaxed model, we begin with a rather large range
and a relatively rough grid.
...
to the |a| unit vector) is forbidden by symmetry for Ir atoms in the 2nd and
4th layers.\ [2]_ ViPErLEED will automatically sort out any symmetry-forbidden
...
to around :math:`\approx 0.21` over the search. We can get a better idea of
how the search has converged by taking a look at the file
:ref:`Search-progress.pdf<searchprogresspdf>` in the ``SUPP`` directory.
...
size (see :numref:`list_ir_displacements_fine`). We now rerun with
...
the **error of the tensor-LEED approximation** used for the structure
optimization. Fortunately — in this case — the real |R factor| (as obtained
from the reference calculation) is lower. However, this is not always the case.
...
at the end of your analysis.\ [3]_
...
ViPErLEED generates a plot of the error curves in ``Errors.pdf`` and stores
the raw data in ``Errors.csv``. The ``Errors.pdf`` file in
:numref:`fig_ir_errors_geo` shows that displacement of surface atoms even by a
few picometers drastically increases the |R factor|. Here, atom 1 is the
...
.. note::
   ...
   :ref:`Errors.pdf<errorspdf>`).

Similarly, changes in the vibrational amplitude of the surface atoms
