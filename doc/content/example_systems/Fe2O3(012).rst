...
replicate the unit cell along :math:`\mathbf{c}` and (optionally) remove the
topmost layer to create a vacuum interface.
In VESTA you can do this similarly, by applying a transformation which
stretches the cell in the :math:`\mathbf{c}` direction, e.g.:
...
all atoms in the first and second layer from -0.05 to 0.05ʁ in steps of
0.01ʁ. We optimize the vibrational amplitudes of the surface atoms in the
same way. We subsequently optimize in-plane position of the first two layers.
