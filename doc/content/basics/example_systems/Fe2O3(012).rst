...
.. literalinclude :: /_static/example_systems/Fe(012)-(1x1)/DISPLACEMENTS_looped
   :language: console
...
By appending the ``1`` to the ``1-3``, we also tell ViPErLEED to perform a
second reference calculation after the optimization with the new structure.
