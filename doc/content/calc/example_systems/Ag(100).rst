...
Files :file:`PARAMETERS_ori` and :file:`POSCAR_ori` (as well as
:file:`VIBROCC_ori` if a :file:`VIBROCC` file was given explicitly)
are the files used as inputs during a specific |calc| run. A copy of these
and all other input files can be found under :file:`SUPP/original_inputs/`.
The :file:`PARAMETERS`, :file:`POSCAR`, and :file:`VIBROCC` files are the
**results** of the |calc| run. This renaming of files is such that |calc|
will automatically use the **outputs** of a previous execution as inputs
for the next one. You can manually invoke the :ref:`bookkeeper` utility
after |calc| if you want a different behavior for a specific run.
...
about each such run. :numref:`list_history_info_ag_init` shows the contents of
the :file:`history.info` file after this first execution of |calc|.
...
The :file:`history.info` can be used as an electronic logbook to keep track
of which calculations were executed.
It is **very good practice** to add some comments to the ``Notes`` section
of each entry. This allows you to quickly recall what was done.
Typical notes for a first run like this one include which sample, which raw
data, and which initial structural model were used for the calculation.
Normally, entries for subsequent runs should be complemented with notes
concerning why the run was executed as well some comments about the outcome.
...
point. As mentioned :ref:`earlier <ag_init>`, this will happen automatically
when |calc| will start the next time.
...
with vibration amplitudes) are saved in the ``OUT`` directory. Interatomic
