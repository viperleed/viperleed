...
.. todo::
    It would be good, perhaps, to report here (or perhaps even earlier) the
    tree resulting from the calculation. It would give us an occasion to
    introduce people to ori and non-ori files, as well as the bookkeeper.
    If we do, we should move the sentence of the next paragraph about history
    to the relevant part.

Now that we found a better configuration, we can use it as our new starting
point. This will happen automatically when |calc| will start the next time.
The results of the previous calculation are stored in a :file:`history` folder
for reference. See the :ref:`bookkeeper` page for more details.
