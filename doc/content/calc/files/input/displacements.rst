...
.. note:: See :ref:`max_tl_displacement` for options to automatically insert reference calculations between searches if displacements become unreasonably large.
...
.. note:: If looped searches are interrupted by :ref:`max_tl_displacement` for a reference calculation and then continued, the previous |R factor| to compare to will be discarded, as very different values may be obtained. Each looped block will therefore again be evaluated at least twice.