    ...
    def getVibAmp(self, rp, chemel):
        ...
        for s in rp.VIBR_AMP_SCALE:
            try:
                ...
                continue
            if value <= 0:
                logger.warning("VIBR_AMP_SCALE parameter: scaling values have "
                        "to be positive floats, ignoring input: "+s)
                continue
            # invDict = {v: k for k, v in rp.ElementAliasDict.items()}         # !!! CLEANUP
            # if self.el in invDict:   #element name changed, replace
            #     label = label.replace(invDict[self.el], self.el, 1)
