...
class BulkSlab(BaseSlab):
    ...
    def apply_bulk_cell_reduction(self, eps, epsz=None,
        ...
        are removed. If the unit cell is actually changed, the
        atom that is currently at the top will also be at the
        top later. Finally, if desired, the z position of the
        new unit cell is centred relative to the geometric
        centre of the atom coordinates.
        ...
        # will 'shift' the fractional ones such that the topmost atom
        # now is still the topmost atom later.
        if not recenter:
            top_atom_cfrac = 0
        elif self.sublayers:
