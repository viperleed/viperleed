...
#    outfilename = 'eeasisss-input-bulk' if bulk else 'eeasisss-input-slab'
    ...
    processed_files = [PSFile(at, filename, ps_energy_step) for (at, filename)
                       in zip(nsl.atlist, filelist)]
    ...
    phaseshift_energies = np.array(
        [psfile.energies for psfile in real_processed_files])
    ...
    atoms_phaseshifts = np.array(
        [psfile.phaseshifts for psfile in real_processed_files])
    ...
    for (site, block_element) in blocks:
        ...
        site_averaged_phaseshifts = np.mean(
            atoms_phaseshifts[considered_atom_indecies, ...], axis=0)

        phaseshift_element = considered_atoms[0].el

        for energy, phaseshifts in zip(
            phaseshift_energies, site_averaged_phaseshifts):
            outvals[energy].append(
                (phaseshift_element, block_element, site,
                 phaseshifts.tolist()))
