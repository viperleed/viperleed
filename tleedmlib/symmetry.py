...
def getSymPosLists(sl, rp, pointlist, output=False):
    ...
    symposlist = list(np.dot(sl.ucell[0:2,0:2],
                            (np.dot(np.linalg.inv(sl.ucell[0:2,0:2]),
                                     np.array(symposlist).transpose())
                               % 1.0)).transpose())
    if len(hexsymposlist) > 0:
        hexsymposlist = list(np.dot(sl.ucell[0:2,0:2],
                            (np.dot(np.linalg.inv(sl.ucell[0:2,0:2]),
...
#        usurf = np.array([[1,0],[0,1]])
    ...
    if not np.array_equal(utr, np.identity(3, dtype=int)):
        ...
        sl.uCellMod.append(('rmul',np.transpose(utr)))
        sl.ucell = np.dot(sl.ucell, np.transpose(utr))
...
def setSymmetry(sl, rp, targetsym):
    ...
    if not '[' in targetsym:
        ...
        if cellchange:
            ...
            rp.setHaltingLevel(1)
            tspar = np.dot(np.linalg.inv(sl.ucell[0:2,0:2]),
                           np.dot(sl.uCellOri[0:2,0:2],tspar))
            for i in range(0,2):
        ...
        if targetsym not in ['pm','pg','cm','rcm','pmg','cmm']:
            ...
            if planegroup == 'rcm':  #reducing to: pg, pm
                ...
                if targetsym == 'pm': #needs origin shift
                    ...
                    sl.uCellMod.append(('add',-shiftv))
                    sl.getFractionalCoordinates()
                    sl.orisymplane.type=SymPlane(np.array([0,0]),
                           np.array(sl.orisymplane.dir[1],
                                    -sl.orisymplane.dir[0]), abst)
                sl.planegroup = targetsym
